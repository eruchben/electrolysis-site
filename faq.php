<?
$this_page = 'faq';
require_once 'header.php';
// require_once 'main_slider.php';
?>

<div class="faq_section container">
    <div class="question">
        <p>What is Electrolysis?</p>
    </div>
    <div class="answer">
        <p>Electrolysis was first invented in 1865 and is a proven, effective and totally safe method of permanent hair removal. It is equally beneficial as a stand alone treatment or as part of a programme of support with hair management systems such as IPL or Laser. Each individual hair requires treatment a number of times, so results take time, however it remains the ONLY PERMANENT method of hair removal. With 3 different methods available there is something to suit everyone.</p>
    </div>
    <div class="question">
        <p>How does the treatment work?</p>
    </div>
    <div class="answer">
        <p>Electrolysis is a progressive treatment meaning that the hair will grow back but will get weaker throughout the course of treatments. During treatment a disposable probe will be introduced into the base of the hair follicle, you will not feel any discomfort. Heat or/and current is then released into the follicle to cut off the blood supply to the hair and to damage any potential for hair growth.. The hair is then removed. As soon as the hair grows through the skin the process is repeated. Eventually the hair will stop growing.</p>
    </div>
    <div class="question">
        <p>Is the treatment painful?</p>
    </div>
    <div class="answer">
        <p>The hair type, location & individuals pain threshold, determines the level of sensation experienced. The benefits of electrolysis are greater than any of the discomfort exerienced!</p>
    </div>
    <div class="question">
        <p>How many treatments will i need?</p>
    </div>
    <div class="answer">
        <p>Electrolysis is a progressive treatment and the ONLY permanent method of hair removal. The amount of treatments will depend upon the cause and size of the hair growth, It is important to follow the treatment plan as advised to obtain the desired results.</p>
    </div>
    <div class="question">
        <p>How will my skin look after treatment?</p>
    </div>
    <div class="answer">
        <p>Following the treatment it is normal for the skin to look pink and feel warm and sensitive. This could last for 24 hours and it is important to follow the aftercare advice given.</p>
    </div>
    <div class="question">
        <p>What can i do with the hairs if i can't make the appointment?</p>
    </div>
    <div class="answer">
        <p>Following the treatment plan is important to obtain the desired results. Do not wax, tweeze or bleach the hairs between treatments. If necessary, trim any hair growth with small sharp scissors.</p>
    </div>
</div>

<? require_once 'footer.php'?>